...
# ----------------------------------------------------------------------------------------------------------------------
font_truetype = ImageFont.truetype("UbuntuMono-R.ttf", size=12, encoding="utf-8") if os.name in ['nt'] else ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", size=12, encoding="utf-8")
# ----------------------------------------------------------------------------------------------------------------------
def gre2jet(rgb):
...
def draw_text(image,label,xy, color_fg,clr_bg=None,font_size=16,alpha_transp=0):
    ...
    total_display_str_height = 1.1 * (font_truetype.getsize(label)[1])
    text_bottom = y if y > total_display_str_height else y + total_display_str_height
    ...
    text_width, text_height = font_truetype.getsize(label)
